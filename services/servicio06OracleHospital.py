...
class ServiceOracleHospital:
    ...
    def eliminarHospital(self, idHospital):
        sql="delete from Hospital where Hospital_no=:idHospital"
        cursor = self.conexion.cursor()        
    ...
    def modificaHospital(self,Hospital_cod , nombre , direccion , telefono , num_cama):
        sql="update Hospital set( hospital_cod = :inumeroHospital , Hospital_no = :inumeroHospital, apellido = :iapellido,  especialidad = :iespecialidad, salario = :isalario) where Hospital_no = :inumeroHospital"
        cursor = self.conexion.cursor()        
        cursor.execute(sql, (Hospital_cod , nombre , direccion , telefono , num_cama))
        registros=cursor.rowcount
        self.conexion.commit()
